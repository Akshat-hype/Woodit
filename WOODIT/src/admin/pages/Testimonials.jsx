...
const Testimonials = () => {
  ...
  const deleteTestimonial = async () => {
    try {
      ...
      toast.error(
        err.response?.data?.message || "Could not delete testimonial",
      );
    } finally {
      setLoading(false);
    }
      ...
      <div className="flex flex-wrap items-center justify-between gap-4">
        <div>
          ...
          <p className="mt-2 text-sm text-[var(--color-text-muted)]">
            Manage client quotes and visibility.
          </p>
        </div>
        <button
          onClick={openCreate}
          ...
          <form onSubmit={saveTestimonial} className="grid gap-4">
            ...
            <label className="block">
              ...
              <textarea
                ...
                onChange={(event) =>
                  updateForm("testimonial_text", event.target.value)
                }
                className="mt-2 min-h-32 w-full rounded-sm border border-[var(--color-border)] px-4 py-3 text-sm outline-none focus:border-[var(--color-primary)]"
                required
              />
            </label>
            <label className="block">
              ...
              <input
                ...
                accept="image/*"
                onChange={(event) =>
                  uploadTestimonialImage(event.target.files?.[0])
                }
              ...
              <div className="mt-2 text-xs text-[var(--color-text-muted)]">
                Uploaded image will be used automatically; manual URLs are
                disabled.
              </div>
              {form.image_url && (
                <div className="mt-2">
