    ...
    <section>
      <div className="flex flex-wrap items-center justify-between gap-4">
        <div>
          ...
          <p className="mt-2 text-sm text-[var(--color-text-muted)]">
            Manage catalogue items, images, featured status, and visibility.
          </p>
        </div>
        <button
          onClick={openCreate}
      ...
      {modalMode && (
        <Modal
          ...
          description="Upload product images, then save the product."
          onClose={closeModal}
        >
          <form onSubmit={saveProduct} className="grid gap-4">
            ...
            <label className="block">
              ...
              <textarea
                ...
                className="mt-2 min-h-28 w-full rounded-sm border border-[var(--color-border)] px-4 py-3 text-sm outline-none focus:border-[var(--color-primary)]"
              />
            </label>
            <label className="block">
              <span className="text-xs font-semibold text-[var(--color-text-muted)]">
            ...
            <div className="block">
              ...
              <div className="mt-2 grid grid-cols-3 gap-2">
                {(form.imagesText || "")
                  ...
                  .map((url) => (
                    <div
                      key={url}
                      className="relative rounded-sm overflow-hidden border border-[var(--color-border)] bg-white"
                    >
                      <img
                        src={url}
                        alt="product"
                      ...
                      <button
                        ...
                        className="absolute top-1 right-1 rounded-sm bg-red-50 p-1 text-red-600"
              ...
              <p className="mt-2 text-xs text-[var(--color-text-muted)]">
                Images must be uploaded via the "Upload Images" field above.
                Manual URLs are disabled.
              </p>
            </div>
            <div className="grid gap-3 sm:grid-cols-2">
              ...
              <label className="flex items-center gap-2 text-sm text-[var(--color-text-muted)]">
                <input
                  ...
                  onChange={(event) =>
                    updateForm("is_visible", event.target.checked)
                  }
                />
                Visible on website
              </label>
