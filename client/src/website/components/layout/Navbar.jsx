    ...
    <header className="sticky top-0 z-50 border-b border-[var(--color-border)] bg-white/95 backdrop-blur">
      <div className="page-shell">
        <div className="flex h-16 items-center justify-between gap-4">
          <Link to="/" onClick={closeMenu} className="flex items-baseline gap-1">
            <span className="font-serif text-2xl font-bold text-[var(--color-text)]">Wood</span>
            <span className="font-serif text-2xl font-bold text-[var(--color-primary)]">It</span>
          </Link>
              ...
              <NavLink
                ...
                className={({ isActive }) =>
                  `text-sm font-medium transition-colors ${
                    isActive ? 'text-[var(--color-primary-dark)]' : 'text-[var(--color-text-muted)] hover:text-[var(--color-text)]'
                  }`
                }
              >
                {link.label}
        ...
        <div className="max-h-[calc(100svh-64px)] overflow-y-auto border-t border-[var(--color-border)] bg-white lg:hidden">
          <div className="page-shell py-5">
            ...
            <div className="mt-4 border-t border-[var(--color-border)] pt-4">
              <p className="mb-2 text-xs font-semibold uppercase tracking-[0.18em] text-[var(--color-primary)]">Collections</p>
